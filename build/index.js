...
class WujieMcpServer {
    ...
    // 处理创建请求
    async handleCreateArtwork(request) {
        ...
        if (!taskKey)
            throw new McpError(ErrorCode.InternalError, "创建任务失败，返回任务key为空");
        let taskData;
    ...
    * @returns 完成的任务数据
    */
    async pollTaskStatus(taskKey, timeout) {
        ...
        // 超时处理
        throw new McpError(ErrorCode.RequestTimeout, `任务超时（${Math.round(timeout / 1000)}秒），请稍后重试`);
    }
    doQueryArtwork(taskKey) {
        return this.wujieAxios.get(`${WUJIE_API_CONFIG.ENDPOINTS.QUERY_TASK}?key=${taskKey}`)
            .then(queryResponse => {
            if (queryResponse.data.code !== "200") {
                throw new McpError(ErrorCode.InternalError, `查询作画结果失败：${JSON.stringify(queryResponse.data)}`);
            }
    ...
    // 处理查询请求
    async handleQueryArtwork(request) {
